 ...
 * 
 * o export.codebase -- a space-separated list of the HTTPMD URLs for use as the
 * codebase. The digest values specified in the URLs will be ignored. The path
 * portion of the URLs, without the message digest parameters, will be used to
 * specify the source files, relative to the source directory, to use for
...
public class MdClassAnnotationProvider extends PreferredClassProvider {
	...
	public MdClassAnnotationProvider() throws IOException,
		...
		try {
			codebaseApp = HttpmdUtil.computeDigestCodebase(System
		...
		try {
			codebaseJsk = HttpmdUtil.computeDigestCodebase(System
