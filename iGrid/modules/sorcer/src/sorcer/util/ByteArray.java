...
/**
 * This class is a container for algorithms working on byte arrays - some of the
 * algorithms are analogous to those in java.lang.String.
 */
public class ByteArray {
	...
	/**
	 * Reads n bytes from the specified input stream. It will return fewer bytes
	 * if fewer bytes are available on the stream. Hence the application should
	 * check the resulting arrays length.
	 */
	...
	/**
	 * Read all input from an InputStream and return as a byte array. This
	 * method will not return before the end of the stream is reached.
	 * 
	...
	/**
	 * Copies data from the specified input stream to the output stream until
	 * end of file is met.
	 * 
	...
	/**
	 * Copies data from the specified input stream to the output stream until
	 * <em>n</em> bytes has been copied or end of file is met.
	 * 
	 * @return the total number of bytes written to the output stream
	 */
	 ...
	 */
	public static final int indexOf(byte[] buf, int ch) {
	 ...
	 */
	public static final int indexOf(byte[] buf, int ch, int fromIndex,
			int toIndex) {
		int i;
	...
	/**
	 * Returns the index of the first occurrence of s in the specified buffer or
	 * -1 if this is not found.
	 */
	 ...
	 * The search starts from fromIndex. This method returns -1 if the index is
	 * not found.
	 */
	...
	/*
	 * // for testing indexOf(byte[], String, int) public static void
	 * main(String[] args) { byte[] buf = getBytes(args[0]);
	 * System.out.println("IndexOf(arg0, arg1, 0) = " + indexOf(buf, args[1],
	 * 3)); }
	 */
	 ...
	 */
	public static final int findBytes(byte buf[], int off, int len, byte ptrn[]) {
	...
	/*
	 * // for testing findBytes(byte[], int, int, byte[]) public static void
	 * main(String args[]) { if (args.length < 4) {
	 * System.err.println("Usage: s1 off len s2"); System.exit(1); } byte b1[] =
	 * new byte[args[0].length()]; byte b2[] = new byte[args[3].length()];
	 * args[0].getBytes(0, args[0].length(), b1, 0); args[3].getBytes(0,
	 * args[3].length(), b2, 0); int off = Integer.parseInt(args[1]); int len =
	 * Integer.parseInt(args[2]); System.out.println("Index = " + findBytes(b1,
	 * off, len, b2)); }
	 */
}
