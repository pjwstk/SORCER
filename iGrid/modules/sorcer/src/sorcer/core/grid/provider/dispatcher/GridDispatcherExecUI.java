...
public class GridDispatcherExecUI extends JFrame implements ActionListener {
    
    private JTextField winExecTfld, winBinFolderTfld, winBinFileTfld, winLibFolderTfld, winLibFileTfld,
	linuxBinFolderTfld, linuxBinFileTfld, solarisBinFolderTfld, solarisBinFileTfld, callercmdTfld, cmdTfld;
    private JLabel winExecLbl, winBinFolderLbl, winBinFileLbl, winLibFolderLbl, winLibFileLbl,
	linuxBinFolderLbl, linuxBinFileLbl, solarisBinFolderLbl, solarisBinFileLbl, callecmdLbl, cmdLbl;
    private JCheckBox isOverWritChkBx;
    private JButton okBtn, cancelBtn, winBtn, solarisBtn, linuxBtn;
    private JComboBox menuCbx;
    
    
    //----------------------------------BUILD GUI BEGIN------------------------------------
    public GridDispatcherExecUI(){//Object obj) {
	super();
	try {
    ...
    private JPanel getMenuUI(){
	JPanel temp = new JPanel(new GridLayout(1,2));
	menuCbx = new JComboBox();
	menuCbx.addItem("Windows");
	menuCbx.addItem("Solaris");
	menuCbx.addItem("Linux");
	menuCbx.setActionCommand("Menu");
	menuCbx.addActionListener(this);
	temp.add(new JLabel("Choose OS: ",JLabel.RIGHT));
	temp.add(menuCbx);	    
	return temp;
    }
	...
	if("Menu".equals(cmd)){
	    String item = (String)menuCbx.getSelectedItem();
	    System.out.println("GridDispatcherExecUI.actionPerformed::item="+item);
	    if("Windows".equals(item)){
		System.out.println("GridDispatcherExecUI.actionPerformed::item matches Windows");
		JFrame temp = new JFrame();
		temp.setTitle("Windows");
		temp.getContentPane().setLayout(new BoxLayout(temp.getContentPane(),BoxLayout.Y_AXIS));
		temp.getContentPane().add(getWinOSUI());
		temp.getContentPane().add(getBtn1UI("Windows", temp));
		temp.pack();
		temp.show();
		temp.setResizable(false);
	    }
	    if("Solaris".equals(item)){
		System.out.println("GridDispatcherExecUI.actionPerformed::item matches Solaris");
		JFrame temp = new JFrame();
		temp.setTitle("Solaris");
		temp.getContentPane().setLayout(new BoxLayout(temp.getContentPane(),BoxLayout.Y_AXIS));
		temp.getContentPane().add(getSolarisOSUI());
		temp.getContentPane().add(getBtn1UI("Solaris", temp));
		temp.pack();
		temp.show();
		temp.setResizable(false);
	    }
	    if("Linux".equals(item)){
		System.out.println("GridDispatcherExecUI.actionPerformed::item matches Linux");
		JFrame temp = new JFrame();
		temp.setTitle("Linux");		
		temp.getContentPane().setLayout(new BoxLayout(temp.getContentPane(),BoxLayout.Y_AXIS));
		temp.getContentPane().add(getLinuxOSUI());
		temp.getContentPane().add(getBtn1UI("Linux", temp));
		temp.pack();
		temp.show();
		temp.setResizable(false);
	    }	       
   ...
   private JPanel getLinuxOSUI(){
	...
	linuxBinFolderLbl = new JLabel("Linux Binary Folder Path: ",JLabel.RIGHT);
	...
	winBinFolderLbl = new JLabel("Windows Binary Folder path: ",JLabel.RIGHT);
	winBinFileLbl = new JLabel("Wiondows Binary File: ", JLabel.RIGHT);
	winLibFolderLbl = new JLabel("Windows Library Folder Path: ", JLabel.RIGHT);
	...
	solarisBinFolderLbl = new JLabel("Solaris Binary Folder Path: ", JLabel.RIGHT);
    ...
    private JPanel getBtn1UI(String opsys, JFrame temp){
	final JFrame temp1 = temp;
	JPanel tempPnl = new JPanel();
	JButton okBtn = new JButton("Save");
	JButton cancelBtn = new JButton("Cancel");
	okBtn.setActionCommand("Save"+opsys);
	cancelBtn.setActionCommand("Cancel"+opsys);
	...
	pnl1.add(new JLabel(""));//Add Nothing to give a space between this and other Guis
