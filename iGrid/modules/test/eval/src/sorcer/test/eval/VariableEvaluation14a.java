...
public class VariableEvaluation14a {
...
//		logger.info("+++++++++++++++++ total derivative y1 wrt x2|y1e2|y1e2g2: " + y1.getTotalDerivative("x2", "y1e2g2"));
	...
	private static VarModel configureAnalysisModel(VarModel model)
		...
		var(model, "xl1",  evaluator("xl1e", "3*x3"), args("x3"));
		// configure response vars
		var(model, "y1", evaluator("y1e1", "xl1 + 2*x2^2 + 3*x3^2 + 5*x4 +4*x2^3"), 
				args("xl1", "x2", "x3", "x4"));
		var(model, "y1", "y1e2", evaluator("y1e2", "y3 + y4 + y5 + 4*x2^3"), 
				args("y3", "y4", "y5", "x2"));
		var(model, "y2", evaluator("y2e", "x2^2"), args("x2"));
		var(model, "y3", evaluator("y3e", "2*y2"), args("y2"));
		var(model, "y4", evaluator("y4e", "xl1 + 3*x3^2"), args("xl1", "x3"));
		var(model, "y5", evaluator("y5e","p1*x4"), args("x4"));
